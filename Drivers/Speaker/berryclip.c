...
static int r_devices_config(void)
{
    ...
    cdev_init(&berryclip_cdev[7], &speaker_fops);
    device[7] = MKDEV(MAJOR(dev_num), MINOR(dev_num));
    device_create(berryclip_class, NULL, device[7], NULL, "speaker");
    cdev_add(&berryclip_cdev[7], device[7], 1);
...
static int r_GPIO_config(void)
{
    ...
    if ((res = gpio_request_one(GPIO_SPEAKER,GPIOF_INIT_LOW,"Speaker")) < 0) {
        printk(KERN_ERR "%s:   GPIO request failure: %s GPIO %d\n",
                KBUILD_MODNAME, "Speaker", GPIO_SPEAKER);
        return res;
    }
    if ((res = gpio_direction_output(GPIO_SPEAKER, 0)) < 0) {
        printk(KERN_ERR "%s:   GPIO set direction output failure: %s GPIO %d\n",
                KBUILD_MODNAME, "Speaker", GPIO_SPEAKER);
        return res;
    }
