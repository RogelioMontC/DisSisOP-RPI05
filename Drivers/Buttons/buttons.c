...
#define MAX_GAP_DEFAULT_JIFFIES 20
...
static ssize_t b_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
    ...
    if (copy_to_user(buf, "1", len)) return -EFAULT; //Cat /dev/locking
...
static int r_init(void)
{
    ...
    printk(KERN_NOTICE "%s: max jiffies: %lu\n", KBUILD_MODNAME, max_jiffies);
...
module_param(max_jiffies,ulong, S_IRUGO);
